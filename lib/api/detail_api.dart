...
getDetails(DetailNotifier detailNotifier) async{
  QuerySnapshot snapshot= await Firestore.instance.collection('appstore').getDocuments();
...
_uploadDetail(Detail detail,selectedDate, {String imageUrl}) async {
  if(imageUrl !=null) {
    CollectionReference detailRef = Firestore.instance.collection('appstore');
...
deleteDetail(Detail detail) async {
  ...
  await Firestore.instance.collection('appstore').document(detail.id).delete();
